...
export class DashboardService {
  ...
  async getBookingTrend(companyId: number, type: 'week' | 'month' = 'week') {
  ...
  async getRevenueTrend(companyId: number, type: 'week' | 'month' = 'week') {
    const startDate = new Date();
  ...
  async getRecentComplaints(companyId: number) {
    const complaints = await this.prisma.complaint.findMany({
      where: {
        companyId,
      },
  ...
  async getSlaAlerts(companyId: number) {
    const now = new Date();
